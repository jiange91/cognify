...
GlobalOptConfig.base_quality = 0.492
GlobalOptConfig.base_price = 19.7
GlobalOptConfig.base_exec_time = 87.3

reasoning_param = reasoning.LMReasoning([NoChange(), ZeroShotCoT()])
model_selection_param = model_selection.model_selection_factory(
    [
        LMConfig(model='fireworks_ai/accounts/zih015-63d1a0/deployedModels/llama-v3p1-8b-instruct-d362cf37', kwargs={'max_tokens': 1024}),
        LMConfig(model='gpt-4o-mini', kwargs={'max_tokens': 1024}),
    ]
)
few_shot_param = LMFewShot(4)
...
agent_names = ["keyword_extraction", "table_selection", "column_selection", "candidate_generation", "revision"]
...
def spwan_agent(agent_name, pool):
    for param in non_structure_cogs:
        ...
        _search_dist[nd_param.hash] = optuna.distributions.CategoricalDistribution(list(nd_param.options.keys()))
...
def build_search_space():
    for module_name in agent_names:
        ...
        _search_dist[d_param.hash] = optuna.distributions.CategoricalDistribution(list(d_param.options.keys()))
...
opt_log_dir = 'one_layer_continue/inner'
...
class OneLayer():
    ...
    def load_ckpt(self):
        ...
        for trial_log_id, trial_log in loaded_logs.items():
            ...
            trial = optuna.trial.create_trial(
                ...
                distributions={k: _search_dist[k] for k in trial_log.params},
            )
            if GlobalOptConfig.quality_constraint is not None:
                add_constraint(trial, score)
                trial.set_system_attr(
                    'constraint_result', get_constraint_result(trial)
                )
    ...
    def eval_workflow(self, structure_decisions, other_decisions, all_decisions):
        ...
        with open(f"{opt_log_dir}/opt_trace.txt", "a") as f:
            f.write(f"- {eval_result.reduced_score}, {eval_result.reduced_price}, {eval_result.reduced_exec_time}\n")
        return eval_result
